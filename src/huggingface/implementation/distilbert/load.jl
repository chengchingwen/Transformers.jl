...
distilbert_pe_indices(pad_id, x) = Base.OneTo{Int32}(size(x, 2)) .+ Int32(pad_id + 1)

function load_model(::Type{HGFDistilBertModel}, cfg, state_dict, prefix)
    ...
    embed = Layers.Chain(CompositeEmbedding(
        token = token,
        position = (position.apply, position.embed, distilbert_pe_indices $ pad_id),
        segment = segment
    ), ln)
    return HGFDistilBertModel(embed, model.encoder, model.pooler)
end

function load_model(
    _type::Type{<:Union{
        HGFDistilBertForCausalLM, HGFDistilBertForMaskedLM, HGFDistilBertForSequenceClassification,
    ...
    return HGFDistilBertModel(embed, encoder, nothing)
end
...
function get_state_dict(m::HGFDistilBertForSequenceClassification, state_dict, prefix)
    get_state_dict(m.model, state_dict, joinname(prefix, "distilbert"))
...
function get_state_dict(m::HGFDistilBertForTokenClassification, state_dict, prefix)
    get_state_dict(m.model, state_dict, joinname(prefix, "distilbert"))
